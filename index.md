...
## Entities
- [[entities/gemini-2.0-flash|Gemini 2.0 Flash]] — Google 推出的轻量级高性能模型，被 Hermes Agent 推荐作为副模型（辅助模型）以降低 Token 消耗
- [[entities/hermes-agent|Hermes Agent]] — Hermes Agent（赫美斯）是一个高度稳定、支持主副模型配置的 AI Agent 框架，通过 Ollama 和 Open WebUI 提供极简部署和优化交互体验。
- [[entities/minimax-m2.7|MiniMax M2.7]] — MiniMax 推出的云端免费模型，通过 Ollama 可直接调用，无需本地 GPU 资源
...
- [[entities/opencloud|OpenCloud]] — OpenCloud（小龙虾）是一个与 Hermes Agent 同类的 AI Agent 项目，但稳定性较差，每次更新都会引入较多 Bug。
...
- [[concepts/token优化|Token 优化]] — Token 优化是通过主副模型配置、辅助任务分配等技术手段，降低 AI Agent 运行过程中 Token 消耗的策略集合。
...
- [[concepts/辅助任务|辅助任务]] — 辅助任务是主副模型配置中由副模型处理的简单任务，包括审批、压缩、MCP 调用、会话搜索、技能检索、网页/视觉相关任务等。
